...
describe('LayoutController', () => {
  it('forwards panel actions to the attached set', () => {
    ...
    service.openProjectFiles()
    service.closeProjectFiles()

    expect(panels.toggleSidebar).toHaveBeenCalledTimes(1)
    expect(panels.openDetails).toHaveBeenCalledTimes(1)
    expect(panels.closeDetails).toHaveBeenCalledTimes(1)
    expect(panels.openProjectFiles).toHaveBeenCalledTimes(1)
    expect(panels.closeProjectFiles).toHaveBeenCalledTimes(1)
  ...
  it('fails loud before the root entry wired its actions', () => {
    ...
    expect(() => { service.openProjectFiles() }).toThrow(/panel actions not wired/)
    expect(() => { service.closeProjectFiles() }).toThrow(/panel actions not wired/)
  })
